...
import (
	...
	"github.com/dingyalin/pinpoint-go-agent/internal/sysinfo"
...
type Config struct {
	...
	Collector struct {
		...
		Uploaded bool
		// UploadedAgentStat
		UploadedAgentStat bool
		// TCPConnTimeout tcp conn timeout
...
var (
	...
	errAgentIDMissing                   = errors.New("string AgentID required")
	errAppNameLimit                     = fmt.Errorf("max of %d length application name", appNameLimit)
	errAgentIDLimit                     = fmt.Errorf("max of %d length agent id", agentIDLimit)
