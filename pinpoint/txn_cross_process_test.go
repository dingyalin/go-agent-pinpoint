...
var (
	replyAccountOne = &internal.ConnectReply{
		CrossProcessID:  "1#1",
		EncodingKey:     "foo",
		TrustedAccounts: map[int]struct{}{1: {}},
	}

	replyAccountTwo = &internal.ConnectReply{
		CrossProcessID:  "2#2",
		EncodingKey:     "foo",
...
func newTxnCrossProcessFromConnectReply(reply *internal.ConnectReply) *txnCrossProcess {
...
func TestTxnCrossProcessInit(t *testing.T) {
	for _, tc := range []struct {
		...
		err := actual.handleInboundRequestHeaders(crossProcessMetadata{
			ID:         id,
			TxnData:    txnData,
