...
func assertTestCaseOutboundHeaders(expect fieldExpect, t *testing.T, hdrs http.Header) {
	...
	// prepare pinpoint header
	nHdr := hdrs.Get("pinpoint")
	decoded, err := base64.StdEncoding.DecodeString(nHdr)
	if err != nil {
		t.Error("failure to decode pinpoint header: ", err)
	}
	nrPayload := struct {
	...
	if err := json.Unmarshal(decoded, &nrPayload); nil != err {
		t.Error("unable to unmarshall pinpoint header: ", err)
	}
	p["pinpoint.v"] = fmt.Sprintf("%v", nrPayload.Version)
	p["pinpoint.d.ac"] = nrPayload.Data.Account
	p["pinpoint.d.ap"] = nrPayload.Data.App
	p["pinpoint.d.id"] = nrPayload.Data.ID
	p["pinpoint.d.pr"] = fmt.Sprintf("%v", nrPayload.Data.Priority)
	p["pinpoint.d.ti"] = fmt.Sprintf("%v", nrPayload.Data.Timestamp)
	p["pinpoint.d.tr"] = nrPayload.Data.TracedID
	p["pinpoint.d.tx"] = nrPayload.Data.TransactionID
	p["pinpoint.d.ty"] = nrPayload.Data.Type
