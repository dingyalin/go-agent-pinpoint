...
func Example() {
	...
	http.HandleFunc(pinpoint.WrapHandleFunc(app, "", func(w http.ResponseWriter, req *http.Request) {
	...
	helloHandler := func(w http.ResponseWriter, req *http.Request) {
		...
		// errors.
		txn := pinpoint.FromContext(req.Context())
	...
	http.HandleFunc(pinpoint.WrapHandleFunc(app, "/hello", helloHandler))
	http.ListenAndServe(":8000", nil)
}

func currentTransaction() *pinpoint.Transaction {
	return nil
}
...
func getApp() *pinpoint.Application {
	return nil
}
...
func ExampleBrowserTimingHeader() {
	...
	http.HandleFunc(pinpoint.WrapHandleFunc(getApp(), "/browser", handler))
	http.ListenAndServe(":8000", nil)
}

func ExampleDatastoreSegment() {
	txn := currentTransaction()
	ds := &pinpoint.DatastoreSegment{
...
func ExampleMessageProducerSegment() {
	...
	seg := &pinpoint.MessageProducerSegment{
		StartTime:       txn.StartSegmentNow(),
		Library:         "RabbitMQ",
		DestinationType: pinpoint.MessageExchange,
...
func ExampleError() {
	...
	// txn.NoticeError(pinpoint.Error{...}) instead of txn.NoticeError(e)
...
func ExampleExternalSegment() {
	...
	segment := pinpoint.StartExternalSegment(txn, request)
...
func ExampleExternalSegment_url() {
	txn := currentTransaction()
	segment := pinpoint.ExternalSegment{
		StartTime: txn.StartSegmentNow(),
...
func ExampleStartExternalSegment_context() {
	...
	request = pinpoint.RequestWithTransactionContext(request, txn)
	segment := pinpoint.StartExternalSegment(nil, request)
...
func ExampleTransaction_SetWebRequest() {
	...
	txn.SetWebRequest(pinpoint.WebRequest{
		Header:    http.Header{},
...
func ExampleTransaction_SetWebResponse() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		r = pinpoint.RequestWithTransactionContext(r, txn)
...
func ExampleTransaction_NewGoroutine_insideGoroutines() {
	...
	async := func(ctx context.Context) {
		...
		txn = txn.NewGoroutine("async")
		defer txn.StartSegment("async").End()

		// Do some work
		time.Sleep(100 * time.Millisecond)
	}
	ctx := pinpoint.NewContext(context.Background(), currentTransaction())
	go async(ctx)
}
