...
import (
	...
	"github.com/dingyalin/pinpoint-go-agent/internal/crossagent"
)
...
func getDTHeaders(app *Application) http.Header {
	hdrs := http.Header{}
	app.StartTransaction("hello").thread.CreateDistributedTracePayload(hdrs, 0)
	return hdrs
}
...
func TestPayloadAcceptAfterCreate(t *testing.T) {
	...
	txn := app.StartTransaction("hello")
	hdrs2 := http.Header{}
	txn.InsertDistributedTraceHeaders(hdrs2, 0)
...
func TestPayloadFromApplicationEmptyTransportType(t *testing.T) {
	...
	//
	// implicit assignment of unexported field 'name' in pinpoint.TransportType literal.
	//
...
func TestPayloadParsingError(t *testing.T) {
	...
	txn.AcceptDistributedTraceHeaders(TransportHTTP, headersFromString(p))
	app.expectSingleLoggedError(t, "unable to accept trace payload", map[string]interface{}{
		"reason": "unable to unmarshal payload data: json: cannot unmarshal array into Go value of type pinpoint.payload",
	})
	txn.End()
	app.expectNoLoggedErrors(t)
	app.ExpectMetrics(t, append([]internal.WantMetric{
...
func TestW3CTraceHeadersNoMatchingNREntry(t *testing.T) {
	...
	txn.InsertDistributedTraceHeaders(outgoingHdrs, 0)
...
func TestDistributedTraceInteroperabilityErrorFallbacks(t *testing.T) {
	// Test what happens in varying cases when both w3c and pinpoint headers
	// are found
	...
	testcases := []struct {
		...
		{
			name:        "w3c present, pinpoint absent, failure to parse traceparent",
			traceparent: "garbage",
		...
		{
			name:        "w3c present, pinpoint absent, failure to parse tracestate",
		...
		{
			name:        "w3c present, pinpoint present, failure to parse traceparent",
			traceparent: "garbage",
			tracestate:  tracestateHdr,
			pinpoint:    pinpointHdr,
			expIntrinsics: map[string]interface{}{
				"guid":     internal.MatchAnything,
				"priority": internal.MatchAnything,
				"sampled":  internal.MatchAnything,
		...
		{
			name:        "w3c present, pinpoint present, failure to parse tracestate",
			traceparent: traceparentHdr,
			tracestate:  "123@nr=garbage",
			pinpoint:    pinpointHdr,
			expIntrinsics: map[string]interface{}{
		...
		{
			name:        "w3c present, pinpoint present",
			traceparent: traceparentHdr,
			tracestate:  tracestateHdr,
			pinpoint:    pinpointHdr,
			expIntrinsics: map[string]interface{}{
		...
		{
			name:        "w3c absent, pinpoint present",
			traceparent: "",
			tracestate:  "",
			pinpoint:    pinpointHdr,
			expIntrinsics: map[string]interface{}{
				...
				"name":                     internal.MatchAnything,
				"parent.transportType":     internal.MatchAnything,
				"parent.type":              "App",                              // from pinpoint header
				"parentSpanId":             "5f474d64b9cc9b2a",                 // from pinpoint header
				"traceId":                  "3221bf09aa0bcf0d3221bf09aa0bcf0d", // from pinpoint header
			},
		},
		{
			name:        "w3c absent, pinpoint absent",
			traceparent: "",
			tracestate:  "",
			pinpoint:    "",
			expIntrinsics: map[string]interface{}{
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			addHdr(hdrs, DistributedTraceNewRelicHeader, tc.pinpoint)
...
func TestW3CTraceIDLengths(t *testing.T) {
	...
	for _, teststr := range testcases {
		...
		out := http.Header{}
		txn.InsertDistributedTraceHeaders(out, 0)
...
func TestUpperCaseTraceIDReceived(t *testing.T) {
	...
	txn.InsertDistributedTraceHeaders(outgoing, 0)
...
func TestDTHeadersAddedTwice(t *testing.T) {
	...
	assertHdrs := func(hdrs http.Header) {
		if h := hdrs[DistributedTraceNewRelicHeader]; len(h) != 1 {
			t.Errorf("incorrect number of pinpoint headers: %#v", h)
		}
