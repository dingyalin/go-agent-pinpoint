...
//
// The pinpoint.Error type, which implements these methods, is the recommended
...
// the following:
//
//	func timeMe(txn pinpoint.Transaction) {
...
func (txn *Transaction) InsertDistributedTraceHeaders(hdrs http.Header, nextSpanID int64) {
	if nil == txn {
		return
	}
	if nil == txn.thread {
	...
	txn.thread.CreateDistributedTracePayload(hdrs, nextSpanID)
}
...
// be reported.
func (txn *Transaction) NewGoroutine(name string) *Transaction {
	if nil == txn {
		return nil
	}
	if nil == txn.thread {
	...
	defer txn.StartSegment("ASYNC/" + name).End()
	return txn.thread.NewGoroutine(name)
}
