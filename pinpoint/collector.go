...
import (
	...
	"github.com/dingyalin/pinpoint-go-agent/internal/logger"
...
// ConnTCP tcp conn
func (pinpointClient *PinpointClient) ConnTCP() error {
	conn, err := net.DialTimeout("tcp", pinpointClient.tcpAddress, pinpointClient.tcpConnTimeout)
	if err != nil {
		pinpointClient.tcpConn = nil
		return err
	}

	pinpointClient.tcpConn = conn
	return nil
}

// ConnStat udp stat conn
func (pinpointClient *PinpointClient) ConnStat() error {
	conn, err := net.Dial("udp", pinpointClient.statAddress)
	if err != nil {
		pinpointClient.statConn = nil
		return err
	}

	pinpointClient.statConn = conn
	return nil
}

// ConnSpan udp span conn
func (pinpointClient *PinpointClient) ConnSpan() error {
	conn, err := net.Dial("udp", pinpointClient.spanAddress)
	if err != nil {
		pinpointClient.spanConn = nil
		return err
	}

	pinpointClient.spanConn = conn
	return nil
}

// WriteTCPData ...
func (pinpointClient *PinpointClient) WriteTCPData(data []byte) error {
	if !pinpointClient.uploaded {
		return nil
	}

	if pinpointClient.tcpConn == nil {
		err := pinpointClient.ConnTCP()
		if err != nil {
			return err
		}
	}

	_, err := pinpointClient.tcpConn.Write(data)
	if err != nil {
		pinpointClient.tcpConn = nil
		return err
	}
...
// WriteStatData ...
func (pinpointClient *PinpointClient) WriteStatData(data []byte) error {
	if !pinpointClient.uploaded {
		return nil
	}

	if pinpointClient.statConn == nil {
		err := pinpointClient.ConnStat()
		if err != nil {
			return err
		}
	}

	_, err := pinpointClient.statConn.Write(data)
	if err != nil {
		pinpointClient.statConn = nil
		return err
	}
...
// WriteSpanData ...
func (pinpointClient *PinpointClient) WriteSpanData(data []byte) error {
	if !pinpointClient.uploaded {
		return nil
	}

	if pinpointClient.spanConn == nil {
		err := pinpointClient.ConnSpan()
		if err != nil {
			return err
		}
	}

	_, err := pinpointClient.spanConn.Write(data)
	if err != nil {
		pinpointClient.spanConn = nil
		return err
	}
...
// SendTCPTStruct ...
func (pinpointClient *PinpointClient) SendTCPTStruct(ttype uint16, tstruct thrift.TStruct) error {
	pinpointClient.messageID++
	messageID := pinpointClient.messageID
	data, err := io.EncodeTCPTTstuctRequest(messageID, ttype, tstruct)
	if err != nil {
		return err
	}
	err = pinpointClient.WriteTCPData(data)
	if err != nil {
		return err
	}

	pinpointClient.logger.Debug("tcp tstruct data", map[string]interface{}{
		"tstruct": fmt.Sprintf("% x", data),
	})
	return nil
}

// SendSpan ...
func (pinpointClient *PinpointClient) SendSpan(tstruct thrift.TStruct) error {
	data, err := io.EncodeTstruct(io.TTypeSpan, tstruct)
	if err != nil {
		return err
	}

	err = pinpointClient.WriteSpanData(data)
	if err != nil {
		return err
	}

	pinpointClient.logger.Debug("span data", map[string]interface{}{
		"tstruct": fmt.Sprintf("% x", data),
	})
	return nil
}
...
// SendSpanChunk ...
func (pinpointClient *PinpointClient) SendSpanChunk(tstruct thrift.TStruct) error {
	data, err := io.EncodeTstruct(io.TTypeSpanChunk, tstruct)
	if err != nil {
		return err
	}
	...
	pinpointClient.logger.Debug("span chunk data", map[string]interface{}{
		"tstruct": fmt.Sprintf("% x", data),
	})
...
// SendAgentStat ...
func (pinpointClient *PinpointClient) SendAgentStat(tstruct thrift.TStruct) error {
	data, err := io.EncodeTstruct(io.TTypeAgentStat, tstruct)
	if err != nil {
		return err
	}

	err = pinpointClient.WriteStatData(data)
	if err != nil {
		return err
	}

	pinpointClient.logger.Debug("agent stat data", map[string]interface{}{
		"tstruct": fmt.Sprintf("% x", data),
	})
