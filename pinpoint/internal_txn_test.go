...
func TestEmptyTransaction(t *testing.T) {
	...
	if tx := txn.NewGoroutine("test"); tx != nil {
		t.Error(tx)
	}
	if m := txn.GetTraceMetadata(); !reflect.DeepEqual(m, TraceMetadata{}) {
		t.Error(m)
...
func TestDTPriority(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			txn.InsertDistributedTraceHeaders(outboundHdrs, 0)
...
func TestErrorAttrsAreOverwritten(t *testing.T) {
	...
	app.ExpectSpanEvents(t, []internal.WantEvent{
		{
			AgentAttributes: map[string]interface{}{
				SpanAttributeErrorClass:   "pinpoint.sampleErrorClass",
...
func Test_encodeTraceID(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "test",
			args: args{"node-app-id", 1532428772942, 77},
			want: []byte{},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := encodeTraceID(tt.args.agentID, tt.args.startTime, tt.args.sequenceID); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("genTransactionID() = %v, want %v", got, tt.want)
			}
		})
	}
}
