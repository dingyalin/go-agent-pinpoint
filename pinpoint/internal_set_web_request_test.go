...
var (
	sampleHTTPRequest = func() *http.Request {
		req, err := http.NewRequest("GET", "http://www.pinpoint.com", nil)
		if nil != err {
			panic(err)
		}
	...
	sampleCustomRequest = func() WebRequest {
		u, err := url.Parse("http://www.pinpoint.com")
		if nil != err {
			panic(err)
		}
	...
	sampleRequestAgentAttributes = map[string]interface{}{
		...
		AttributeRequestURI:           "http://www.pinpoint.com",
	}
)
...
func TestSetWebRequestWithDistributedTracing(t *testing.T) {
	...
	app := testApp(distributedTracingReplyFields, enableBetterCAT, t)
	hdrs := http.Header{}
	app.StartTransaction("hello").InsertDistributedTraceHeaders(hdrs, 0)
	...
	app.ExpectTxnEvents(t, []internal.WantEvent{{
		AgentAttributes: map[string]interface{}{
			"request.method":       "GET",
			"request.uri":          "http://www.pinpoint.com",
