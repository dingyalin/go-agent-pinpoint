...
//
// Example: https://github.com/pinpoint/go-agent/tree/master/v3/integrations/nrgin/example/main.go
package nrgin
...
func (w *replacementResponseWriter) WriteString(s string) (int, error) {
	w.flushHeader()
	// w.code = w.ResponseWriter.Status()
...
// found.
func Transaction(c Context) *pinpoint.Transaction {
	if v := c.Value(internal.GinTransactionContextKey); nil != v {
		if txn, ok := v.(*pinpoint.Transaction); ok {
			return txn
		}
	}
...
func Middleware(app *pinpoint.Application) gin.HandlerFunc {
...
func MiddlewareHandlerTxnNames(app *pinpoint.Application) gin.HandlerFunc {
	return middleware(app, false)
}

func middleware(app *pinpoint.Application, useNewNames bool) gin.HandlerFunc {
		...
		if app != nil {
			...
			defer func() {
				...
				if err != nil {
					...
					repl.code = http.StatusInternalServerError
				}
				repl.flushHeader()
				txn.End()
