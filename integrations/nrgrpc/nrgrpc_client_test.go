...
import (
	...
	"github.com/dingyalin/pinpoint-go-agent/integrations/nrgrpc/testapp"
...
func TestUnaryClientInterceptor(t *testing.T) {
	...
	if hdr, ok := hdrs["pinpoint"]; !ok || len(hdr) != 1 || "" == hdr[0] {
...
func TestClientUnaryMetadata(t *testing.T) {
	...
	if hdr, ok := hdrs["pinpoint"]; !ok || len(hdr) != 1 || "" == hdr[0] || "payload" == hdr[0] {
		t.Error("distributed trace header not sent", hdrs)
	}
