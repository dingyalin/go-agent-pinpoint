...
func (t *TestHandler) Method(ctx context.Context, req *TestRequest, rsp *TestResponse) error {
	...
	defer pinpoint.FromContext(ctx).StartSegment("Method").End()
	return nil
}
...
func getDTRequestHeaderVal(ctx context.Context) string {
	if md, ok := metadata.FromContext(ctx); ok {
		if dtHeader, ok := md[pinpoint.DistributedTraceNewRelicHeader]; ok {
			return dtHeader
		}
...
var cfgFn = func(cfg *pinpoint.Config) {
	cfg.Attributes.Include = append(cfg.Attributes.Include,
		pinpoint.AttributeMessageRoutingKey,
		pinpoint.AttributeMessageQueueName,
		pinpoint.AttributeMessageExchangeType,
		pinpoint.AttributeMessageReplyTo,
		pinpoint.AttributeMessageCorrelationID,
	)
}

func newTestWrappedClientAndServer(app *pinpoint.Application, wrapperOption client.Option, t *testing.T) (client.Client, server.Server) {
...
func testClientCallMetadata(c client.Client, t *testing.T) {
	// test that context metadata is not changed by the pinpoint wrapper
	req := c.NewRequest(serverName, "TestHandler.Method", &TestRequest{}, client.WithContentType("application/json"))
	rsp := TestResponse{}
	app := createTestApp()
	txn := app.StartTransaction("name")
...
func TestClientPublishWithNoTransaction(t *testing.T) {
	...
	if _, err := b.Subscribe(topic, func(e broker.Event) error {
		defer wg.Done()
		h := e.Message().Header
		if _, ok := h[pinpoint.DistributedTraceNewRelicHeader]; ok {
	...
	app.ExpectSpanEvents(t, []internal.WantEvent{
		{
			...
			UserAttributes: map[string]interface{}{},
			AgentAttributes: map[string]interface{}{
				pinpoint.SpanAttributeErrorClass:   "401",
				pinpoint.SpanAttributeErrorMessage: "Unauthorized",
	...
	app.ExpectSpanEvents(t, []internal.WantEvent{
		{
			...
			UserAttributes: map[string]interface{}{},
			AgentAttributes: map[string]interface{}{
				pinpoint.SpanAttributeErrorClass:   "*errors.errorString",
				pinpoint.SpanAttributeErrorMessage: "subscriber error",
				"message.routingKey":               "topic",
			},
		},
	})
...
func newTestClientServerAndBroker(app *pinpoint.Application, t *testing.T) (client.Client, server.Server, broker.Broker) {
	b := bmemory.NewBroker()
	c := client.NewClient(
		client.Broker(b),
