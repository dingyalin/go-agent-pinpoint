...
func StartSegment(input StartSegmentInputs) *http.Request {
	...
	if input.ServiceName == "dynamodb" || input.ServiceName == "DynamoDB" {
		...
		segment = pinpoint.StartExternalSegment(txn, input.HTTPRequest)
	}

	integrationsupport.AddAgentSpanAttribute(txn, pinpoint.SpanAttributeAWSOperation, input.Operation)
	integrationsupport.AddAgentSpanAttribute(txn, pinpoint.SpanAttributeAWSRegion, input.Region)
...
func EndSegment(ctx context.Context, hdr http.Header) {
	if segment, ok := ctx.Value(segmentContextKey).(endable); ok {
		if id := GetRequestID(hdr); "" != id {
			...
			integrationsupport.AddAgentSpanAttribute(txn, pinpoint.SpanAttributeAWSRequestID, id)
		}
		segment.End()
	}
}
